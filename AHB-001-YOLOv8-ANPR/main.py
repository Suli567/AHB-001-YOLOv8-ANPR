...
license_plate_detector = YOLO('anpr.pt')
...
while ret:
    ...
    if ret:
        ...
        for detection in detections_:
            ...
            bbox = np.array([x1, y1, x2 - x1, y2 - y1]).reshape(1, -1)

            # Update the tracker with this bounding box
            track_ids = mot_tracker.update(bbox)

             # Draw the bounding box and the class name on the frame
